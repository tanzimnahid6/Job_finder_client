...
const JobCard = ({ job }) => {
  ...
  const { title, deadline, salary, type, _id } = job;
  const handleJobDelete = (_id) => {
    dispatch(asyncDeleteJob(_id));
    Swal.fire({
      position: "top",
      icon: "warning",
        ...
        <div>
          ...
          <div className="flex-col flex md:flex-row gap-4 text-xs md:text-base my-5 md:my-0">
            <div className="flex gap-1">
              <span className="md:hidden inline">Type :</span> {typeOfJob}
            </div>
            <div className="space-x-4">
              <span>Salary: $ {salary}</span>
              <span>
                Closing <span className="md:inline hidden">on</span> :{deadline}
              </span>
            </div>
          </div>
        </div>
